...
<script type="text/javascript">
	...
	function displayDate() {
		(function() {
			...
			var optionLoop = function(start, end, id, datestr) {
				...
				for (i = end; i >= start; i--) {
					if (!dateStrFlg
							&& (datestr == 'yyyy年' || datestr == 'mm月' || datestr == 'dd日')) {
						opt += "<option value='" + datestr + "' selected>"
								+ datestr + "</option>";
						dateStrFlg = true;
					}
	...
	function checkID() {
		...
		window.open("/kimschool_spring/login/getUserID?userId=" + userId.value,
				"popupView",
				"top=200,left=300,width=400,height=400,resizable=no");
	}
</script>
</head>
<body>
	...
	<c:if test="${isInsertUserFormFlg == true}">
		<c:if test="${isUserIDFlg == true}">
			<script type="text/javascript">
				alert('すでに存在するIDです。新しいIDを入力してください');
			</script>
		</c:if>
		...
		<div>
			<form action="/kimschool_spring/login/insertUser" method="post"
				...
				<p>
					ID<br> <input type="text" name="userId" id="userId"> <span">必須</span>
					<input type="button" value="ID確認" onclick="checkID()">
				</p>
				<p>
					氏名（漢字）<br> <input type="text" name="lastname_kanji"
						id="lastname_kanji" placeholder="氏"> <input type="text"
						name="firstname_kanji" id="firstname_kanji" placeholder="名">
					<span">必須</span>
				</p>
				<p>
					氏名（フリガナ）<br> <input type="text" name="lastname_hurigana"
						id="lastname_hurigana" placeholder="氏"> <input type="text"
						name="firstname_hurigana" id="firstname_hurigana" placeholder="名">
					<span">必須</span>
				</p>
				<p>
					性別<br> <input type="radio" name="sex" value="M">男 <input
						type="radio" name="sex" value="F">女 <span">必須</span>
				</p>
				<p>
					生年月日<br> <select name="year" id="year"></select> <select
				...
				<p>
					PASSWORD<br> <input type="password" name="password"
						id="password" placeholder="8桁以上の半角英数字"> <span">必須</span>
				</p>
				<p>
					PASSWORD（確認）<br> <input type="password" id="password_confirm"
						placeholder="確認のため、もう一度入力してください"> <span">必須</span>
				</p>
				<p>
					LINE ID<br> <input type="text" name="lineId">
				</p>
				<p>
					KaKaoTalk ID<br> <input type="text" name="kakaoId">
				</p>
				<p>
